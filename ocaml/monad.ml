...
module Make(M : Base) = struct
  ...
  let (>>=)       = bind 
  let (>=>) g f x = bind (g x) ~f:f
  let (<=<) f g x = bind (g x) ~f:f

  let lift1 f x     = bind x ~f:(fun x -> return (f x))
  let lift2 f x y   = bind x ~f:(fun x -> lift1 (f x) y)
...
struct
  ...
  let null     = M.null
  let filter p xs =
    bind xs ~f:(fun x -> if p x then return x else zero ())
  let of_list xs = List.fold_left
    (fun x y -> plus x (return y))
    (zero  ()) xs
  let sum xs =
    bind xs ~f:(fun xs -> List.fold_right plus (List.map return xs) (zero ()))
...
struct
  ...
  include MakePlus(struct
                    type 'a m = 'a err
                    let map x ~f:f = match x with
                        Error e -> Error e
                      | Result x -> Result (f x)
                    let return x = Result x
                    let bind x ~f:f = match x with
                        Error  e -> Error e
                      | Result x -> f x
