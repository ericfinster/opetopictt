...
type value =
  | FlexV of mvar * spine
  | RigidV of lvl * spine
  | TopV of name * spine * value Lazy.t
...
let rec eval top loc tm =
  match tm with
  | VarT i -> db_get i loc
  | TopT nm -> TopV (nm,Emp,lazy (assoc nm top))
...
let rec quote k v ufld =
  ...
  | TopV (_,_,tv) when ufld -> quote k (Lazy.force tv) ufld
...
let rec unify stgy top l t u =
  ...
  | (TopV (_,_,tv) , TopV (_,_,tv')) when Poly.(=) stgy UnfoldAll ->
    unify UnfoldAll top l (Lazy.force tv) (Lazy.force tv')
  ...
  | (TopV (_,sp,tv) , TopV (_,sp',tv')) when Poly.(=) stgy OneShot ->
    (try unifySp UnfoldNone top l sp sp'
     with Unify_error _ -> unify UnfoldAll top l (Lazy.force tv) (Lazy.force tv'))
      
  | (TopV (_,_,_) , _) when Poly.(=) stgy UnfoldNone ->
    raise (Unify_error "refusing to unfold top level def")
  | (TopV (_,_,tv) , t') -> unify stgy top l (Lazy.force tv) t'
  | (_ , TopV (_,_,_)) when Poly.(=) stgy UnfoldNone ->
    raise (Unify_error "refusing to unfold top level def")
  | (t , TopV (_,_,tv')) -> unify stgy top l t (Lazy.force tv')
